use crate::{
    burn_tokens, calculate_health_factor, error::CustomError, get_lamport_from_usd, withdraw_sol,
...
pub fn process_liquidate(ctx: Context<Liquidate>, amount_to_burn: u64) -> Result<()> {
    ...
    let lamports = get_lamport_from_usd(&amount_to_burn, &ctx.accounts.price_feed)?;
